...
const Contact = () => {
    ...
    return (
      <section className="bg-dark_primary text-white" id="contact">
        <div className="md:container px-5 py-14 animate-pulse">
          <div className="h-10 bg-slate-700/50 rounded-lg w-40 mb-4 mx-auto md:mx-0"></div>
          <div className="h-6 bg-slate-700/50 rounded-lg w-1/3 mb-10 mx-auto md:mx-0"></div>
          <div className="flex gap-10 md:flex-row flex-col">
            <div className="flex-1 flex flex-col gap-4">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="h-14 bg-slate-700/50 rounded-lg w-full"></div>
              ))}
              <div className="h-36 bg-slate-700/50 rounded-lg w-full"></div>
            </div>
            <div className="flex-1 flex flex-col gap-6 justify-center">
              {[1, 2, 3].map(i => (
                <div key={i} className="h-16 bg-slate-700/50 rounded-xl w-full"></div>
              ))}
            </div>
          </div>
        </div>
  ...
  const ContactData = dbContent?.contact || staticContent.Contact;
  ...
  // Extract whatsapp number if available from dynamic data
  const whatsappNumber = ContactData.social_media?.find(m => m.text?.toLowerCase().includes("whatsapp"))?.link?.replace("https://wa.me/", "") || "96103635086";
            ...
            {ContactData.social_media?.map((content, i) => (
              ...
              >
                <span className="text-2xl text-white group-hover:scale-110 transition-transform duration-300">
                  {typeof content.icon === 'string' ? createElement(iconMap[content.icon] || GrMail) : createElement(content.icon)}
                </span>
